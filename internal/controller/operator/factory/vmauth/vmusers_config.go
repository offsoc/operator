...
func buildConfig(ctx context.Context, rclient client.Client, vmauth *vmv1beta1.VMAuth, pos *parsedObjects, ac *build.AssetsCache) ([]byte, error) {
	// loads info about exist operator object kind for crdRef.
	crdCache := fetchCRDRefURLs(ctx, rclient, pos)
	toCreateSecrets, toUpdate, err := addAuthCredentialsBuildSecrets(pos, ac)
	if err != nil {
		return nil, err
	}

	// generate yaml config for vmauth.
	cfg, err := generateVMAuthConfig(vmauth, pos, crdCache, ac)
	if err != nil {
		return nil, err
	}
...
func addAuthCredentialsBuildSecrets(pos *parsedObjects, ac *build.AssetsCache) (needToCreateSecrets []*corev1.Secret, needToUpdateSecrets []*corev1.Secret, resultErr error) {
...
func fetchCRDRefURLs(ctx context.Context, rclient client.Client, pos *parsedObjects) map[string]string {
	crdCacheURLCache := make(map[string]string)
	pos.users.ForEachCollectSkipInvalid(func(user *vmv1beta1.VMUser) error {
		if len(user.Spec.TargetRefs) == 0 {
			return fmt.Errorf("has no target refs")
		}
		for j := range user.Spec.TargetRefs {
			ref := user.Spec.TargetRefs[j]
			if ref.CRD == nil {
...
func generateVMAuthConfig(cr *vmv1beta1.VMAuth, pos *parsedObjects, crdCache map[string]string, ac *build.AssetsCache) ([]byte, error) {
